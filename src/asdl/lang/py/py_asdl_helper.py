...
# from https://stackoverflow.com/questions/15357422/python-determine-if-a-string-should-be
        ...
        if field.cardinality == 'single' or field.cardinality == 'optional':
            if field_value is not None:  # sometimes it could be 0
                if grammar.is_composite_type(field.type):
                    ...
                    # Removed the string conversion. With the Constant AST node in python 3.8,
                    # the value of the node is in its correct dtype.
...
def asdl_ast_to_python_ast(asdl_ast_node, grammar):
    ...
    for field in asdl_ast_node.fields:
        ...
        if grammar.is_composite_type(field.type):
            ...
            # FIXME: hack! if int? is missing value in ImportFrom(identifier? module,
            #  alias* names, int? level), fill with 0
