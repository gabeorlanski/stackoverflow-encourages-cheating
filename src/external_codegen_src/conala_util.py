"""
This code comes from https://github.com/neulab/external-knowledge-codegen/blob/master/datasets
/conala/common.py
"""
...
def is_enumerable_str(identifier_value):
    ...
    return len(identifier_value) > 2 and identifier_value[0] in ('{', '(', '[') and \
           identifier_value[-1] in ('}', ']', ')')
...
def canonicalize_code(code, slot_map):
    ...
    # Intent: zip two lists `[1, 2]` and `[3, 4]` into a list of two tuples containing elements
